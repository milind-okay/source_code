...
void power(long long int F[2][2], long long int n)
{
    if(n==1 || n==0)
      return;
      power(F,n/2);
     long long  int x =  F[0][0]*F[0][0] + F[0][1]*F[1][0];
 long long int y =  F[0][0]*F[0][1] + F[0][1]*F[1][1];
 long long int z =  F[1][0]*F[0][0] + F[1][1]*F[1][0];
  long long int w =  F[1][0]*F[0][1] + F[1][1]*F[1][1];
     F[0][0] = x;
  F[0][1] = y;
  F[1][0] = z;
  F[1][1] = w;
